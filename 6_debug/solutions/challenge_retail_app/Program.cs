...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded > 0)
    {
        // Add change back to till
        cashTill[3] += changeTwenties;
        cashTill[2] += changeTens;
        cashTill[1] += changeFives;
        cashTill[0] += changeOnes;
        
        // Remove amount paid by customer from till and return to customer
        cashTill[3] -= twenties;
        cashTill[2] -= tens;
        cashTill[1] -= fives;
        cashTill[0] -= ones;
